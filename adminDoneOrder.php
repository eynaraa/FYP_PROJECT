...
echo '<div class="header">
  ...
  if ($_SERVER['REQUEST_METHOD']=='POST')
  {
    ...
    {
      //make the query
      $q = "DELETE FROM cust WHERE custID = $id LIMIT 1";
      $result = @mysqli_query ($connect, $q); //run the query
      
      if (mysqli_affected_rows($connect)==1)//if there was a problem
      //display message
      {
        echo'<script>alert("Order has been completed");
        window.location.href="adminOrder.php";</script>';
      }
      else
      {
    ...
    {
      echo '<script>alert("Order has NOT been completed");
      window.location.href="adminOrder.php";</script>';
    }
  }
  else
  {
    ...
    $q = "SELECT custName FROM cust WHERE custID = $id";
    ...
    if (mysqli_num_rows($result) ==1)
    {
      ...
      $row =  mysqli_fetch_array($result, MYSQLI_NUM);
            echo'
            <div class="base">
                <h2 class="h2">Done Order Record</h2>
            </div>';
      echo "<h3 class=h3>Are you sure the order of $row[0] is complete? <br>This order will be permanently deleted.</h3>";
      echo '<form action="adminDoneOrder.php" method="post">
